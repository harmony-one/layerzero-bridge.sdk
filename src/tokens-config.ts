...
export const getTokenConfig = (addr: string, tokenType: TOKEN, tokenNetwork: NETWORK_TYPE): ITokenInfo => {
  let token: ITokenInfo;

  if ([TOKEN.ERC20, TOKEN.ERC721].includes(tokenType)) {
  ...
  if ([TOKEN.ONE, TOKEN.ETH].includes(tokenType)) {
    token = tokensConfigs.find(
      t =>
        t.type === tokenType &&
        t.network === tokenNetwork,
    );
  }
